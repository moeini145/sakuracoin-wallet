...
public final class BitcoinIntegration
{
	...
	{
		final StringBuilder uri = new StringBuilder("sakuracoin:");
		if (address != null)
			uri.append(address);
		if (amount != null)
	...
	{
		Toast.makeText(context, "No Sakuracoin application found.\nPlease install Sakuracoin Wallet.", Toast.LENGTH_LONG).show();

		final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=ja.ohac.wallet"));
		final Intent binaryIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://ohac.github.io/sakuracoin-wallet/"));
