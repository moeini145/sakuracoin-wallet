...
public class ExchangeRatesProvider extends ContentProvider
{
	...
	public Cursor query(final Uri uri, final String[] projection, final String selection, final String[] selectionArgs, final String sortOrder)
	{
		...
		if (exchangeRates == null || now - lastUpdated > UPDATE_FREQ_MS)
		{
			...
			if (newExchangeRates == null)
				newExchangeRates = requestExchangeRates(BITCOINAVERAGE_URL, sakuraBtcConversion, BITCOINAVERAGE_FIELDS);
			if (newExchangeRates == null)
				newExchangeRates = requestExchangeRates(BITCOINCHARTS_URL, sakuraBtcConversion, BITCOINCHARTS_FIELDS);
			if (newExchangeRates == null)
				newExchangeRates = requestExchangeRates(BLOCKCHAININFO_URL, sakuraBtcConversion, BLOCKCHAININFO_FIELDS);
		...
		if (exchangeRates == null || sakuraBtcConversion == -1)
			return null;
	...
	private static Map<String, ExchangeRate> requestExchangeRates(final URL url, float sakuraBtcConversion, final String... fields)
	{
						...
						for (final String field : fields)
						{
							...
							if (rate != null)
							{
								try
								{
									...
									{
										rates.put(currencyCode, new ExchangeRate(currencyCode, sakuraRate, url.getHost()));
										break;
									}
								}
